...
export default function AccidentDetails() {
  ...
  const fetchAccidentDetails = async () => {
    try {
      const response = await axiosApi.get(
          " http://localhost:5173/api/Accidents"
      );
  ...
  const onViewPhotos = async (accident) => {
    try {
      const response = await axiosApi.get(` http://localhost:5173/api/accidents/photos/${accident.accidentId}`);
  ...
  const onConfirmDelete = async () => {
    try {
      ...
      const endpoint = ` http://localhost:5173/api/Accidents/${
