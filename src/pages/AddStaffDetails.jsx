...
export default function AddStaffDetails() {
  ...
  const handleSubmit = async (values, actions) => {
    ...
    try {
      const response = await axiosApi.post(' http://localhost:5173/api/Staff', allValues);
      console.log(response)
      if (response.status === 200) {
        if (response.data.status) {
          setModalMessage('Staff added successfully. User credentials have been sent!');
          setIsModalOpen(true);
        } else {
      ...
      setModalMessage(`Failed to add staff: ${error.message}`);
      setIsModalOpen(true);
    }
  };
