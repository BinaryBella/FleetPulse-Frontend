...
export default function EditAccidentDetails() {
  ...
  const fetchAccidentDetails = async () => {
    try {
      const response = await axiosApi.get(` http://localhost:5173/api/Accidents/${id}`);
  ...
  const fetchDriverNICs = async () => {
    try {
      const response = await axiosApi.get(" http://localhost:5173/api/Auth/drivers/nics");
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const response = await axiosApi.put(` http://localhost:5173/api/Accidents/${id}`, values);
