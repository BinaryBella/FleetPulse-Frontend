...
const EditVehicleMaintenanceConfiguration = () => {
    ...
    const fetchMaintenanceConfiguration = async () => {
        try {
            const response = await axiosApi.get(` http://localhost:5173/api/VehicleMaintenanceConfiguration/${id}`);
            setInitialValues({
    ...
    const handleSubmit = async (values) => {
        try {
            ...
            const response = await axiosApi.put(` http://localhost:5173/api/VehicleMaintenanceConfiguration/${id}`, payload);
